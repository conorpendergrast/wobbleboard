...
- An [**Intercom**](https://www.intercom.com) account where you have admin rights (a free trial workspace works). All three regions (US, EU, AU) are supported via the `INTERCOM_REGION` env var.
...
The Intercom client picks its base URL from the `INTERCOM_REGION` env var. Set it in `.env.local` to match your workspace before running any sync scripts:

- US (default): `INTERCOM_REGION=us` → `https://api.intercom.io`
- EU: `INTERCOM_REGION=eu` → `https://api.eu.intercom.io`
- AU: `INTERCOM_REGION=au` → `https://api.au.intercom.io`

If unset, the client defaults to US. An invalid value throws on the first request.
...
- **Intercom sync fails with `Resource Not Found` from `api.intercom.io`.** Your workspace is in the EU or AU region but `INTERCOM_REGION` is unset (or set to `us`). See [Intercom region](#intercom-region-eu--au-workspaces).
